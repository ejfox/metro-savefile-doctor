...
export default function App({ saveFilePath, outputPath, createBackup }) {
    ...
    const getFieldValue = (field) => {
        switch (field) {
            case 'money':
                return saveData.data.money;
            case 'elapsedSeconds':
                return saveData.data.elapsedSeconds;
            case 'ownedTrainCount':
                return saveData.data.ownedTrainCount || 0;
            case 'transitCost':
                return saveData.data.transitCost || 0;
            default:
                return null;
        }
    };
    ...
    if (editingField) {
        return (
            <Box flexDirection="column">
                ...
                <Box>
                    <Text>New value: </Text>
                    <TextInput value={editValue} onChange={setEditValue} onSubmit={finishEditing} />
                </Box>
                <Newline />
    ...
    return (
        <Box flexDirection="column" padding={1}>
            ...
            <Box flexDirection="column">
                ...
                <Text dimColor>
                    Stations: {saveData.data.stations?.length || 0} | Routes: {saveData.data.routes?.length || 0} |
                    Trains: {saveData.data.trains?.length || 0}
                </Text>
            </Box>
            <Newline />
